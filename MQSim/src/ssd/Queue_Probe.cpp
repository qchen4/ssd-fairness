...
namespace SSD_Components
{
	...
	void Queue_Probe::setCount(int val)
	{
		...
		// Initialize lastCountChangeReference on first count change
		if (lastCountChangeReference == 0 && n > 0) {
			lastCountChangeReference = n;
		}
	...
	void Queue_Probe::Snapshot(std::string id, Utils::XmlWriter& writer)
	{
		...
		for (unsigned int i = 0; i < states.size(); i++) {
			...
			// Avoid division by zero: if lastCountChangeReference is not initialized or equals now, use a safe default
			double r = 0.0;
			sim_time_type timeDiff = now - lastCountChangeReference;
			if (timeDiff > 0) {
				r = (double)t / (double)timeDiff;
			} else if (now > 0) {
				// Fallback: use now as denominator if lastCountChangeReference wasn't initialized
				r = (double)t / (double)now;
			}
	...
	double Queue_Probe::AvgQueueLength()
	{
		...
		sim_time_type currentTime = Simulator->Time();
		if (currentTime == 0) {
			return 0.0;
		}
		return (double)sum / (double)currentTime;
	}
	...
	double Queue_Probe::STDevQueueLength()
	{
		...
		double mean = (double)sum / (double)currentTime;
		double stdev = 0.0;
		for (unsigned int len = 0; len < states.size(); len++) {
			stdev += std::pow((double)states[len].totalTime * len / (double)currentTime - mean, 2);
		}
	...
	double Queue_Probe::AvgQueueLengthEpoch()
	{
		...
		sim_time_type epochDuration = Simulator->Time() - epochStartTime;
		if (epochDuration <= 0) {
			return 0.0;
		}
		return (double)sum / (double)epochDuration;
	}
