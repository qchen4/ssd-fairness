...
namespace SSD_Components
{
	...
	void TSU_FLIN::Schedule()
	{
			...
			it != transaction_receive_slots.end(); it++)
		{
			...
			bool priority_class_resolved = false;
			unsigned int priority_class = 0;
			auto resolve_priority_class = [&]() -> unsigned int
			{
				if (!priority_class_resolved)
				{
					priority_class = static_cast<unsigned int>((*it)->Priority_class);
					if (priority_class >= no_of_priority_classes)
					{
						priority_class = no_of_priority_classes - 1;
					}
					priority_class_resolved = true;
				}
			...
			case Transaction_Type::READ:
				...
				{
					unsigned int priority_class = resolve_priority_class();
	...
	{
		auto select_user_queue = [&](void) -> Flash_Transaction_Queue*
		{
			if (scheduling_turn_assignments_read.empty())
				return nullptr;
			unsigned int attempts = 0;
			unsigned int turn_count = (unsigned int)scheduling_turn_assignments_read.size();
			unsigned int turn = current_turn_read;
			while (attempts < turn_count)
			{
				unsigned int priority_class = scheduling_turn_assignments_read[turn] % no_of_priority_classes;
				if (UserReadTRQueue[chip->ChannelID][chip->ChipID][priority_class].size() > 0)
				{
					current_turn_read = (turn + 1) % turn_count;
					return &UserReadTRQueue[chip->ChannelID][chip->ChipID][priority_class];
				}
				turn = (turn + 1) % turn_count;
				attempts++;
			}
			return nullptr;
		};

		Flash_Transaction_Queue *mapping_queue = &MappingReadTRQueue[chip->ChannelID][chip->ChipID];
		Flash_Transaction_Queue *gc_queue = &GCReadTRQueue[chip->ChannelID][chip->ChipID];
		Flash_Transaction_Queue *user_queue = select_user_queue();
		Flash_Transaction_Queue *sourceQueue1 = nullptr;
		Flash_Transaction_Queue *sourceQueue2 = nullptr;

		bool gc_urgent = ftl->GC_and_WL_Unit->GC_is_in_urgent_mode(chip);

		if (mapping_queue->size() > 0)
		{
			sourceQueue1 = mapping_queue;
			if (gc_urgent && gc_queue->size() > 0)
				sourceQueue2 = gc_queue;
			else
				sourceQueue2 = user_queue;
		}
		else if (gc_urgent)
		{
			if (gc_queue->size() > 0)
			{
				sourceQueue1 = gc_queue;
				sourceQueue2 = user_queue;
			}
			else if (user_queue != nullptr)
			{
				sourceQueue1 = user_queue;
			}
			else
				return false;
		}
		...
		{
			if (user_queue != nullptr)
			{
				sourceQueue1 = user_queue;
				if (gc_queue->size() > 0)
					sourceQueue2 = gc_queue;
			}
			else if (gc_queue->size() > 0)
			{
				sourceQueue1 = gc_queue;
			}
			else
	...
	bool TSU_FLIN::service_write_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		auto select_user_queue = [&](void) -> Flash_Transaction_Queue*
		{
			if (scheduling_turn_assignments_write.empty())
				return nullptr;
			unsigned int attempts = 0;
			unsigned int turn_count = (unsigned int)scheduling_turn_assignments_write.size();
			unsigned int turn = current_turn_write;
			while (attempts < turn_count)
			{
				unsigned int priority_class = scheduling_turn_assignments_write[turn] % no_of_priority_classes;
				if (UserWriteTRQueue[chip->ChannelID][chip->ChipID][priority_class].size() > 0)
				{
					current_turn_write = (turn + 1) % turn_count;
					return &UserWriteTRQueue[chip->ChannelID][chip->ChipID][priority_class];
				}
				turn = (turn + 1) % turn_count;
				attempts++;
			}
		...
		Flash_Transaction_Queue *gc_queue = &GCWriteTRQueue[chip->ChannelID][chip->ChipID];
		Flash_Transaction_Queue *user_queue = select_user_queue();
		Flash_Transaction_Queue *sourceQueue1 = nullptr;
		Flash_Transaction_Queue *sourceQueue2 = nullptr;
	...
	bool TSU_FLIN::service_erase_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		...
		return issue_command_to_chip(sourceQueue, NULL, Transaction_Type::ERASE, false);
	}
