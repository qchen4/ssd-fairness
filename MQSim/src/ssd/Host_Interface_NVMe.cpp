...
{
	auto dispose_request = [](User_Request* request, const char* queue_name)
	{
		if (request->Transaction_list.size() != 0)
		{
			std::cerr << "Warning: skipping destruction of user request " << request->ID
				<< " in " << queue_name << " queue because " << request->Transaction_list.size()
				<< " flash transactions remain outstanding" << std::endl;
			return;
		}
		delete request;
	};
	...
	for (auto &user_request : Waiting_user_requests)
		dispose_request(user_request, "waiting");
	for (auto &user_request : Completed_user_requests)
		dispose_request(user_request, "completed");
}
