	...
	try
	{
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			if (strcmp(param->name(), "Seed") == 0) {
				...
				else if (strcmp(val.c_str(), "QFQ") == 0)
				{
					Transaction_Scheduling_Policy = SSD_Components::Flash_Scheduling_Type::QFQ;
				}
				else if (strcmp(val.c_str(), "MINMAX") == 0)
				{
					Transaction_Scheduling_Policy = SSD_Components::Flash_Scheduling_Type::MINMAX;
				}
				else if (strcmp(val.c_str(), "RR") == 0)
				{
					Transaction_Scheduling_Policy = SSD_Components::Flash_Scheduling_Type::RR;
				}
				else if (strcmp(val.c_str(), "DRR") == 0)
				{
					Transaction_Scheduling_Policy = SSD_Components::Flash_Scheduling_Type::DRR;
				}
				else {
