...
#include "../ssd/TSU_QFQ.h"
#include "../ssd/TSU_MinMax.h"
#include "../ssd/TSU_RR.h"
#include "../ssd/TSU_DRR.h"
...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
{
	...
	case NVM::NVM_Type::FLASH:
	{
		...
		case SSD_Components::Flash_Scheduling_Type::OUT_OF_ORDER:
			...
			break;
		case SSD_Components::Flash_Scheduling_Type::QFQ:
			tsu = new SSD_Components::TSU_QFQ(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2 *>(device->PHY),
											  parameters->Flash_Channel_Count, parameters->Chip_No_Per_Channel,
											  parameters->Flash_Parameters.Die_No_Per_Chip, parameters->Flash_Parameters.Plane_No_Per_Die,
											  erase_suspension, program_suspension,
											  parameters->Preferred_suspend_write_time_for_read, parameters->Preferred_suspend_erase_time_for_read,
		...
		{
			constexpr unsigned int priority_class_count = IO_Flow_Priority_Class::NUMBER_OF_PRIORITY_LEVELS;
			auto stream_count_per_priority_class = new unsigned int[priority_class_count]();
			for (auto &flow : (*io_flows))
			{
				auto priority_index = static_cast<unsigned int>(flow->Priority_Class);
				if (priority_index >= priority_class_count)
				{
					throw std::invalid_argument("FLIN scheduler does not support the configured priority class.");
				}
				stream_count_per_priority_class[priority_index]++;
			}

			auto stream_ids_per_priority_class = new stream_id_type *[priority_class_count];
			for (unsigned int i = 0; i < priority_class_count; i++)
			{
				stream_ids_per_priority_class[i] = new stream_id_type[stream_count_per_priority_class[i]];
			}

			std::vector<unsigned int> class_position(priority_class_count, 0);
			for (stream_id_type flow_id = 0; flow_id < io_flows->size(); flow_id++)
			{
				auto priority_index = static_cast<unsigned int>((*io_flows)[flow_id]->Priority_Class);
				stream_ids_per_priority_class[priority_index][class_position[priority_index]++] = flow_id;
			}
			...
			tsu = new SSD_Components::TSU_FLIN(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2 *>(device->PHY),
											   ...
											   10000000, 33554432, 262144, priority_class_count, (unsigned int)io_flows->size(), stream_count_per_priority_class, stream_ids_per_priority_class,
											   0.6,
		...
		case SSD_Components::Flash_Scheduling_Type::MINMAX:
			tsu = new SSD_Components::TSU_MinMax(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2*>(device->PHY),
				parameters->Flash_Channel_Count, parameters->Chip_No_Per_Channel,
				parameters->Flash_Parameters.Die_No_Per_Chip, parameters->Flash_Parameters.Plane_No_Per_Die,
				parameters->Preferred_suspend_write_time_for_read, parameters->Preferred_suspend_erase_time_for_read,
				parameters->Preferred_suspend_erase_time_for_write,
				erase_suspension, program_suspension);
			break;
		case SSD_Components::Flash_Scheduling_Type::RR:
			tsu = new SSD_Components::TSU_RR(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2*>(device->PHY),
				parameters->Flash_Channel_Count, parameters->Chip_No_Per_Channel,
				parameters->Flash_Parameters.Die_No_Per_Chip, parameters->Flash_Parameters.Plane_No_Per_Die,
				erase_suspension, program_suspension,
				parameters->Preferred_suspend_write_time_for_read, parameters->Preferred_suspend_erase_time_for_read,
				parameters->Preferred_suspend_erase_time_for_write);
			break;
		case SSD_Components::Flash_Scheduling_Type::DRR:
			tsu = new SSD_Components::TSU_DRR(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2*>(device->PHY),
				parameters->Flash_Channel_Count, parameters->Chip_No_Per_Channel,
				parameters->Flash_Parameters.Die_No_Per_Chip, parameters->Flash_Parameters.Plane_No_Per_Die,
				erase_suspension, program_suspension,
				parameters->Preferred_suspend_write_time_for_read, parameters->Preferred_suspend_erase_time_for_read,
