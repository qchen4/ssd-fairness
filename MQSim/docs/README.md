...
## Automated Scheduler Experiments

The repository ships with [`run_experiments.sh`](run_experiments.sh), a helper script that sweeps multiple transaction schedulers across several representative workloads and collates the outputs.

### Default workloads

| File | Description |
| --- | --- |
| `workload_scenario_1.xml` | Write-heavy synthetic stress (two NVMe flows). |
| `workload_scenario_2.xml` | Read-dominated synthetic workload for latency sensitivity. |
| `workload_scenario_3.xml` | Mixed trace-based workload (`traces/tpcc-small.trace`). |
| `fast18/backend-contention/workload-backend-contention-flow-1-flow-2.xml` | Two-flow backend contention study from the FAST’18 artifact. |

You can edit the `WORKLOADS` array near the top of the script to add/remove scenarios or point to your own XML definitions.
...
The script ensures MQSim is compiled, iterates over the scheduling policies (`OUT_OF_ORDER`, `PRIORITY_OUT_OF_ORDER`, `FLIN`, `QFQ`, `MINMAX`) and each workload, and places the artifacts under `results/<timestamp>/<policy>/<workload>/`. Every run log is stored as `run.log`, and the freshly generated `MQSim_Results*.xml` (plus any `*_results` folders produced by MQSim) are copied alongside it. When the sweep finishes, the script also archives the entire timestamped directory into `results/<timestamp>.zip` for easy sharing.

### MINMAX stream weights

The current MINMAX scheduler uses per-stream weights defined in [`src/ssd/TSU_MinMax.h`](src/ssd/TSU_MinMax.h) (`FlowState::weight`, default `1.0`). To evaluate weighted fairness, adjust the initializer or add custom logic before running the automation script. The script itself does not modify these weights; documenting your chosen values alongside the experiment results is recommended.
...
21. **Transaction_Scheduling_Policy:** the transaction scheduling policy that is used in the SSD back end. Range = {OUT_OF_ORDER as defined in the Sprinkler paper [2], PRIORITY_OUT_OF_ORDER which implements OUT_OF_ORDER and NVMe priorities, QFQ (Quick Fair Queueing) for per-stream weighted fairness}.
